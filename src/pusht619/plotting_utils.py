...
def plot_perturbation_hist(
    ...
    face_choices = np.argmax(x_all[:, :, :NUM_FACES], axis=-1).flatten()

    n_cols = NUM_FACES + 3  # face logits + cp + angle + argmax
    fig, axes = plt.subplots(1, n_cols, figsize=(3 * n_cols, 3))
    fig.suptitle(f"c perturbations — iter {iteration}", fontsize=9)

    for fi in range(NUM_FACES):
        ax = axes[fi]
        ax.hist(c_pert_all[:, :, fi].flatten(), bins=20, color="steelblue", alpha=0.7)
        for env_i in range(n_envs):
            ax.axvline(c_np[env_i, fi], color="red", linewidth=1.5, label="c" if env_i == 0 else None)
    ...
    chosen_unperturbed = int(np.argmax(np.asarray(x_star)[0, :NUM_FACES]))
    ax_face.axvline(chosen_unperturbed, color="red", linewidth=1.5, label="x_star")
    ax_face.set_xticks(range(NUM_FACES))
    ax_face.set_title("argmax face (perturbed)", fontsize=8)
    ax_face.set_xlabel("face index", fontsize=7)
    ax_face.legend(fontsize=7)

    lo_cp, hi_cp = CONTACT_POINT_BOUNDS
    lo_ang, hi_ang = float(ANGLE_BOUNDS[0]), float(ANGLE_BOUNDS[1])

    ax_cp = axes[NUM_FACES + 1]
    ax_cp.hist(c_pert_all[:, :, NUM_FACES].flatten(), bins=20, color="steelblue", alpha=0.7)
    for env_i in range(n_envs):
    ...
    ax_cp.set_xlabel("value", fontsize=7)

    ax_ang = axes[NUM_FACES + 2]
    ax_ang.hist(c_pert_all[:, :, NUM_FACES + 1].flatten(), bins=20, color="steelblue", alpha=0.7)
    for env_i in range(n_envs):
    ...
    ax_ang.set_xlabel("value", fontsize=7)
