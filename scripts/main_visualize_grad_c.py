...
def plot_c_and_grads_by_face(iterations, c_env, c_env_perturbed, c_env_cost, c_env_perturbed_costs, n_mc_envs, n_envs, save_path):
    ...
    for i in range(NUM_FACES):
        ...
        for j in range(NUM_FACES):
            ...
            ax.plot(iterations, c_vals, label=f"Face {i} Logit (c)", color="blue", linewidth=2)
...
def main():
    ...
    assert all_grad_c_face.shape == (n_iterations, n_envs, ACTION_DIM)
    ...
    for env_idx in range(n_envs):
        ...
        plot_c_and_grads_by_face(
            iterations=iterations, c_env=env_c, c_env_perturbed=env_c_perturbed, c_env_cost=env_current_c_dists, c_env_perturbed_costs=env_costs_per_env, n_mc_envs=n_mc_envs, n_envs=n_envs, save_path=f"{base_path}__per_face.png"
        )
