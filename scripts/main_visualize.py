...
python scripts/main_visualize.py
"""
...
from pusht619.core import Action, PushTEnv, ANGLE_BOUNDS, CONTACT_POINT_BOUNDS, NUM_FACES, get_action_cost_matrix_cache_filename, ACTION_COST_MATRIX_CACHE_DIR
...
def draw_scene_visualization(ax_scene, env):
    """Draws the T block, target T block, and face labels on the given axes."""
    t_corners, target_corners = env.get_t_and_target_corners()
    ...
    from pusht619.core import FACE_START_POINTS, FACE_END_POINTS, T_CORNERS
    for face_idx in range(NUM_FACES):
        ...
        ax_scene.text(text_pos[0], text_pos[1], f"Face {face_idx}", 
...
def main():
    ...
    # We need a fixed initial state to evaluate all actions from the same starting point
    env.reset(seed=0)
    target_poses = env.target_poses.copy()
    t_poses = env.t_poses.copy()
    t_vel = env.t_velocities.copy()
    cache_file = get_action_cost_matrix_cache_filename(t_poses[0], t_vel[0], target_poses[0], N_SIM_STEPS, n_grid)
    ...
    draw_scene_visualization(axes[-1], env)
